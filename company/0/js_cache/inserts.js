...
function fix_date(date,last){var regex=/(\d+)[^\d]*(\d+)*[^\d]*(\d+)*/;var dat=regex.exec(last);var cur=regex.exec(date);var day,month,year;if(!dat||!cur)return date;if(cur[3]!=undefined)
dat=cur;if(user.datefmt==0){day=dat[2];month=dat[1];year=dat[3];}else if(user.datefmt==1){day=dat[1];month=dat[2];year=dat[3];}else{day=dat[3];month=dat[2];year=dat[1];}
if(cur[2]==undefined)
day=cur[1];else
if(cur[2]!=undefined){if(user.datefmt==1){day=cur[1];month=cur[2]}
else{day=cur[2];month=cur[1]}
}
if(day<10)day='0'+parseInt(day,10);if(month<10)month='0'+parseInt(month,10);if(year<100)year=year<60 ?(2000+parseInt(year,10)):(1900+parseInt(year,10));if(user.datefmt==0)
return month+user.datesep+day+user.datesep+year;if(user.datefmt==1)
...
tab.onclick=function(){_expand(this);return false;}
}
},
...
else if(ev.altKey&&!ev.ctrlKey&&((key>47&&key<58)||(key>64&&key<91))){key=String.fromCharCode(key);var n=_hotkeys.focus;var l=document.getElementsBySelector('[accesskey='+key+']');var cnt=l.length;_hotkeys.list=l;for(var i=0;i<cnt;i++){n=(n+1)%cnt;if(l[n].accessKey==key&&l[n].scrollWidth){_hotkeys.focus=n;var tmp=function(){l[_hotkeys.focus].focus();};setTimeout(tmp,0);break;}
}
return stopEv(ev);}
